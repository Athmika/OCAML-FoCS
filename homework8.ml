...
let neg_int = ("neg_int","<x -> pair (not (pfirst x)) (psecond x)>")

let plus_int = ("plus_int","<a -> <b -> if (and (pfirst a) (pfirst b)) (pair true (plus (psecond a) (psecond b)))  
	                                       (if (and (pfirst a) (not (pfirst b))) 
	                                       (
	                                       	if (geq (psecond a) (psecond b))
	                                       	    (pair true (minus (psecond a) (psecond b)))
	                                       	    (pair false (minus (psecond b) (psecond a)))
	                                       	) 
	                                       ((if (and (not (pfirst a)) (pfirst b)) 
	                                       	(
	                                       	if (geq (psecond a) (psecond b)) 
	                                       		(pair false (minus (psecond a) (psecond b))) 
	                                       		(pair true (minus (psecond b) (psecond a)))
	                                       	) 
	                                       (pair false (plus (psecond a) (psecond b)))
	                                        ))) >>")


let times_int = ("times_int","<a -> <b -> if (and (pfirst a) (pfirst b)) (pair true (times (psecond a) (psecond b))) 
	                                      (
	                                      if (and (not(pfirst a)) (not(pfirst b))) 
	                                      (pair true (times (psecond a) (psecond b))) 
	                                      (pair false (times (psecond a) (psecond b)))
	                                      )  
...
let triple = ("triple", "<a  -> <b -> <c -> <s -> s a b c >>>>")
let tfirst = ("tfirst", "<t -> t <a -> <b -> < c-> a>>>>")
let tsecond = ("tsecond", "<t -> t <a -> <b -> < c-> b>>>>")
let tthird = ("tthird", "<t -> t <a -> <b -> < c-> c>>>>")
...
let match_list = ("match_list","<L -> <a -> <f -> if (pfirst (L)) (a) (f (pfirst (psecond L)) (psecond (psecond L)))>>>")

let length = ("length"," Y <length -> <L -> if (pfirst L) _0 (succ (length(psecond (psecond L)))) >>")

let sum = ("sum"," Y <sum -> <L -> if (pfirst L) _0 (plus (pfirst (psecond L)) (sum(psecond (psecond L))))>>")


let append = ("append","Y <append -> <L1 -> <L2 -> if (pfirst L1) (L2) (append (psecond (psecond L1)) (cons (pfirst (psecond L1)) L2)) >>>")


let map = ("map","Y <map -> <f -> < L -> if (pfirst L) (empty) (cons (f (pfirst (psecond L))) (map f (psecond (psecond L)) )) >>>")



let q3_defs : (string * string) list = 
   default_defs @ [empty; cons; match_list; length; 
                   sum; append; map]
